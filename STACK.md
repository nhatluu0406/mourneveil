...
- Production asset authority: editable glTF sources live under `assets/source`, validated committed runtime imports under `public/assets`, and typed runtime references use stable `/assets/...` URLs; M8's first contract is glTF 2.0 in meters, Y-up, ground-centered, with provenance/license recorded in `assets/production-assets.json`; render meshes never author physics, which remains an explicit world-owned proxy
