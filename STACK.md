...
- Room composition: one canonical dungeon definition (ADR-0004) compiles rooms and object instances into render, collision, nav, lights, and interactions. ADR-0003 rooms remain the authoring unit. ADR-0002 registry instantiates compiled placements. Gameplay zone IDs stay in `connectedLevel`. A structural object is never authored independently in render and physics.
...
- Production visual authority: project-authored code-native geometry is allowed where M10 benefits from custom silhouettes; each default production candidate has a stable ID/provenance record in `productionVisualLedger.ts`, stays presentation-only, and is measured by the deterministic 1440×900 hero renderer gate; imported binary assets continue through the M8 source/runtime manifest and byte budgets; modular world objects follow ADR-0002/ADR-0004 (dungeon → rooms → instances → type catalog → render/collision; no class-inheritance object framework). Production room files must not create meshes, PointLights, or CuboidColliders.
- World shadow policy: the mounted route defaults to source lighting plus contact/value grounding without a full-route directional shadow pass; actors and rare explicit landmarks may remain shadow-capable for scoped hero views, while ordinary static architecture, practical fixtures, and small dressing do not cast dynamic shadows. Shadow/DPR alternatives remain development-only measured modes, never gameplay authority.
...
- Connected-level collision: the dungeon compiler derives Rapier box colliders from the same object instances that produce visible structure; gate colliders are present exactly while their simulation-owned world flags are closed; there is no second handwritten wall map. Enemy pursuit uses direct motion when clear, short-lived deterministic corner detours around compiled blocking footprints, and immutable route anchors across zone connections — Rapier still resolves every movement step; no navmesh or A* engine
...
- Application integration → session intent (Continue / New Rite / `?fresh=1`) → `game/runtime` session coordinator → player/combat/enemy/encounter/world/item/save domain runtimes. Runtime is not constructed until session intent is resolved. New Rite clears canonical and legacy save keys and starts a fresh rite. Reload with a save is Continue, not an implicit new game.
