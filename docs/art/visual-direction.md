...
## M11 boss encounter language

The Veilbound Sepulchre is an ossuary containment warden that outlived its rite: a broad dark-iron body inside asymmetric mortuary plate, bronze chains and crown/reliquary spines, a single heavy censer-maul, and cyan-white veil fractures. At phase two, sealed plates physically open around the exposed core rather than relying on a color swap. The arena uses a clearly visible dark-mid funerary floor over the unchanged collision proxy, warm ritual fire against rare spectral lamps, and a radial containment seal with a clear melee center. Boss presentation outranks location/objective chrome while compact survival and item state remain visible.

M11 evidence is bounded at **340 connected-route draw calls, 80k triangles, 180 geometries, 16 textures, 14 programs, 540 objects, 330 meshes, 12 lights, and 160 MB heap**. The boss gate separately permits a measured peak of 360 draw calls; repeated combat must show no texture/mesh/light growth and at most bounded one-time geometry compilation.

## M13 progression and Court language

Progression uses three non-interchangeable funerary motifs: Vitality is an ember-crimson vessel, Resolve a pale bone/verdigris ward, and Might an ash-gold oathblade. Character UI projects resolved runtime contributions and compact comparisons; normal HUD adds only level and a thin XP line. Vitality Charm and Ward Seal use distinct authored glyphs. The Court of Quiet Names adds pointed/broken lancet profiles, burial clusters, split stonework, chains, a double funeral sconce, low ember vessel, and spectral reliquary; sparse actual lights remain attached to visible sources while shared geometry/materials control fragmentation.

M13 MB2 measured route peaks: refuge **309 draw calls / 147 geometries**, Court **306 / 154**, Ash Walk **330 / 177**; all use **3 textures, 12–13 programs, 489 objects, 283 meshes, 12 lights, ~97 MB reported heap**. Repeated combat showed zero geometry/texture/mesh/light growth.

M13 MB4 gives active oaths separate visual grammar: Veil Step uses torn cyan shards, Oath Cleave bronze oath-force streaks, and Ward Pulse pale body-local containment facets. Their procedural poses and effects project the authoritative action snapshot only. Oath & Armory prioritizes a horizontal three-oath row and fits without native scroll at 1440×900; only the relic list may scroll at 1280×720. The Refuge–Court frame uses darker profiled arches and memorial blockers, burial screens, processional ember markers, and lifted floor/masonry midtones. Final measured route peaks are **311 / 311 / 332 draw calls** and **148 / 154 / 178 geometries** (Refuge/Court/Ash), with **3 textures, 12–13 programs, 505 objects, 295 meshes, 12 lights, ~97 MB heap** and zero repeated-combat resource growth.
